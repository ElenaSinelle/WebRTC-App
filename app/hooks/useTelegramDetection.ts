...
export const useTelegramDetection = () => {
  const telegramInfo = useMemo(() => {
    ...
    const userAgent = navigator.userAgent;
    console.log(' UserAgent for detection:', userAgent);

    const hasTelegramInUA = userAgent.includes('Telegram') || userAgent.includes('TelegramBot');

    const hasTelegramWebviewProxy = !!window.TelegramWebviewProxy;
    const hasTelegram = !!window.Telegram;
    const hasTelegramWebApp = hasTelegram && !!window.Telegram?.WebApp;

    const isTelegram = hasTelegramInUA || hasTelegramWebviewProxy || hasTelegramWebApp || hasTelegram;
    ...
    if (isTelegram) {
      if (hasTelegramWebviewProxy) webViewType = 'webview-proxy';
      else if (hasTelegramWebApp) webViewType = 'webapp';
      else if (isTelegramIOS) webViewType = 'ios-wkwebview';
      else if (isTelegramAndroid) webViewType = 'android-webview';
      else webViewType = 'desktop';
    }

    console.log(' Telegram detection result:', {
      isTelegram,
      isTelegramWebView: isTelegram && (isTelegramIOS || isTelegramAndroid || !!hasTelegramWebviewProxy),
      isTelegramDesktop,
      isTelegramIOS,
      isTelegramAndroid,
      webViewType,
